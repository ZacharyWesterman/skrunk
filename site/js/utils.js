...
window.lookups = {
	...
	get max_displayed_pages() {
		const page_max_width_px = parseFloat(getComputedStyle(document.querySelector('.page')).maxWidth)
		const content_max_width_px = parseFloat(getComputedStyle(document.documentElement).width)

		const max_width_px = Math.min(page_max_width_px, content_max_width_px)
		const two_rem = 2 * parseFloat(getComputedStyle(document.documentElement).fontSize)

		const max_pages = max_width_px / two_rem - 6
	...
	prev_pages: (index, total) => {
		...
		if (index >= page_ct / 2) {
			return (index - Math.floor(page_ct / 2))
		}
	...
	next_pages: (index, total) => {
		const page_ct = Math.ceil(lookups.max_displayed_pages)
		const min_pages = lookups.prev_pages(index, total)

		if (min_pages + page_ct >= total) {
			return total
		}

		return min_pages + page_ct
	},
}
