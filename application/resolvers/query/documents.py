...
from application.db.documents import get_document, get_child_documents
...
def resolve_get_document(_, info: GraphQLResolveInfo, id: str) -> dict:
	"""
...
@perms.module('documents')
def resolve_get_child_documents(_, info: GraphQLResolveInfo, id: str | None) -> list[dict]:
	"""
	...
	Args:
		_ (Any): Placeholder.
		info (GraphQLResolveInfo): Information about the GraphQL execution state.
		id (str | None): The ID of the parent document for which to retrieve child documents. If None, retrieves top-level documents.
