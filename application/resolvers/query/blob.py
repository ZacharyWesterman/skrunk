...
@perms.module('files')
def resolve_get_blobs(_, info: GraphQLResolveInfo, filter: BlobSearchFilter, start: int, count: int, sorting: Sorting) -> dict:
	try:
...
@perms.module('files')
def resolve_count_blobs(_, info: GraphQLResolveInfo, filter: BlobSearchFilter) -> dict:
	try:
		user_data = perms.caller_info()
...
def resolve_get_blob(_, info: GraphQLResolveInfo, id: str) -> dict:
	return get_blob_data(id)


@query.field('totalBlobSize')
@perms.module('files')
def resolve_total_blob_size(_, info: GraphQLResolveInfo, filter: BlobSearchFilter) -> dict:
	try:
		user_data = perms.caller_info()
...
@perms.module('files')
def resolve_process_qr_from_blob(_, info: GraphQLResolveInfo, id: str) -> str | None:
	try:
...
@perms.module('files')
def resolve_count_tag_uses(_, info: GraphQLResolveInfo, tag: str) -> int:
	group = userids_in_groups(perms.caller_info().get('groups', []))
	return count_tag_uses(tag, group)


@query.field('generateUID')
def resolve_generate_uid(_, info: GraphQLResolveInfo) -> str:
	return get_uid()
...
@perms.module('files')
@handle_client_exceptions
def resolve_poll_zip_progress(_, info: GraphQLResolveInfo, uid: str) -> dict:
